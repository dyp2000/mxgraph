...
class mxUtils
{
    ...
    const INFO      = 0; // blue
    const SUCCESS   = 1; // green
    const WARNING   = 2; // yellow
    const ERROR     = 3; // red
    const DEBUG     = 4; // gray
    const INFO_BG       = 'rgba(0,0,255,.1)';
    const INFO_CLR      = 'blue';
    const SUCCESS_BG    = 'rgba(0,255,0,.1)';
    const SUCCESS_CLR   = 'green';
    const WARNING_BG    = 'rgba(255,190,0,.1)';
    const WARNING_CLR   = '#a87e00';
    const ERROR_BG      = 'rgba(255,0,0,.1)';
    const ERROR_CLR     = 'red';
    const DEBUG_BG      = 'rgba(128,128,128,.1)';
    const DEBUG_CLR     = 'black';

    static function dd($type, $text, $val = null, $file = null, $line = null, $die = true)
    {
        switch ($type) {
            case self::INFO:
            ...
            case self::SUCCESS:
                ...
                $color = self::SUCCESS_CLR;
                break;
            case self::WARNING:
                $background = self::WARNING_BG;
                $color = self::WARNING_CLR;
                break;
            case self::ERROR:
                $background = self::ERROR_BG;
                $color = self::ERROR_CLR;
                break;
            default:
                $background = self::DEBUG_BG;
                $color = self::DEBUG_CLR;
                break;
        }
        print('<pre style="border-radius: 4px; border-left: solid 6px '.$color.'; padding: 8px; background-color: '.$background.'; color: '.$color.'">');
        print('[');
        $str = empty($file) ? '' : $file;
        !empty($str) ? print($str.':') : print($str);
        $str = empty($line) ? '' : strval($line);
        print($str."]\n\n".$text);
        var_dump($val);
        $die ? die() : false;
        print('</pre>');
