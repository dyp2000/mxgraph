...
class mxGdCanvas
{
     ...
     */
    function __construct($width = 0, $height = 0, $scale = 1, $background = null, $imageBasePath = "")
    {
    ...
    function drawCell($state)
    {
        ...
        if (isset($state->absolutePoints) && sizeof($state->absolutePoints) > 1) {
            ...
            if (mxUtils::getValue($style, mxConstants::$STYLE_SHAPE, "") != mxConstants::$SHAPE_SWIMLANE) {
                ...
                $start = mxUtils::getNumber($style, mxConstants::$STYLE_STARTSIZE, mxConstants::$DEFAULT_STARTSIZE) * $this->scale;

                // Removes some styles to draw the content area
                $cloned = array_slice($style, 0);

                // mxUtils::dd(mxUtils::DEBUG, 'cloned = ', $cloned, __FILE__, __LINE__, false);

                $cloned[mxConstants::$STYLE_FILLCOLOR] = isset($cloned[mxConstants::$STYLE_SWIMLANE_FILLCOLOR]) ? $cloned[mxConstants::$STYLE_SWIMLANE_FILLCOLOR] : '#ffffff';
